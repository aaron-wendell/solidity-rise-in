...
contract ProposalContract {
    ...
    function calculateCurrentState() private view returns(bool) {
        ...
        uint256 quorum_threshold = proposal.total_vote_to_end / 2; // 50% quorum requirement

        // Check if quorum is met
        if (total_votes < quorum_threshold) {
            return false; // Proposal fails if quorum is not met
        }

        // Adjust pass votes (similar to original logic)
        if (pass % 2 == 1) {
            pass += 1;
        }
        pass = pass / 2;

        // Determine if approve votes exceed reject + adjusted pass votes
